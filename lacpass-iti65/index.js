...
// Practitioner REAL: lee los Provider Attributes de OpenMRS (PRN/PPN/NI/practitioner_type) y los
// mapea al recurso. ANTES esto pisaba todo con datos hardcodeados de demo ("Marcelo Salas",
// PPN CL987654, MD K7L8M) → el IPS cruzaba la frontera con un profesional inventado.
async function normalizePractitionerResource(prac) {
    return enrichPractitionerFromOpenmrs(prac);
}
function normalizeOrganizationResource(orga) {
    if (!orga || orga.resourceType !== 'Organization') return;


    // RUT REAL del establecimiento (antes era 'G7H8', un valor inventado de demo). Mismo dato,
    // type y system que el default del fhir-forwarder-ips-mediator → consistente entre mediadores.
    const identifiers = [
        {
            "use": "official",
            "type": {
    ...
    // Dirección real del establecimiento. OJO: 'Región Metropolitana' es la REGIÓN (va en `state`),
    // no la ciudad; la ciudad es Santiago.
    const address = [
        {
            "line": [
    ...
    // Nombre oficial completo — debe coincidir con el resto de los mediadores (SR/contrarreferencia/MeOw
    // usan 'Hospital Clínico San Borja Arriarán'); antes acá decía 'Hospital San Borja' (inconsistente).
    orga.name = 'Hospital Clínico San Borja Arriarán';
...
app.post('/lacpass/_iti65', async (req, res) => {
  ...
  try {
    ...
    await normalizePractitionerResource(practitionerEntry?.resource);
